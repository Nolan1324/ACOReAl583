...
#
#)
TESTS=("00204.c" "maximum_sum_rectangle.cpp")

#setup
COMPILER="clang++"
FLAGS="-O3 -Rpass=register-allocation -Rpass-analysis -fno-inline -lm -g"
REG_ALLOC="-mllvm -regalloc=aco -mllvm -aco-spill-cost-importance=0 -mllvm -aco-max-time=10 -mllvm -aco-num-ants=40" #set register allocator
UNIQUE_ID=$(date +%s.%N)
DIRECTORY_NAME="aco_nocost_run_$UNIQUE_ID" # change this to not overwrite results of previous run, or use unique id
