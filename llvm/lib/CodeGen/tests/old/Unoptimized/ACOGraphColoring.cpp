...
void profileStop() {
    ...
    double active_time_ns = active_duration.count();  // Nanoseconds as a raw value
    double total_time_ns = total_interval.count();    // Nanoseconds as a raw value
    ...
    double percentage = (active_time_ns / total_time_ns) * 100.0;
    ...
    if (call_count % print_frequency == 0) {
        double average_percentage = cumulative_percentage / print_frequency;
...
static double assignmentWeight(int vertex, int color, Parameters& params, vector<vector<int>>& neighborsByColor, Solution& solution, const vector<vector<double>>& pheromones) {
    profileStart();
    double w1 = fastPow(pheromoneTrail(vertex, color, solution, pheromones), params.alpha);
    profileStop();
    double w2 = fastPow(heuristic(vertex, color, neighborsByColor), params.beta);
    double w3 = params.allowedColors[vertex][color];
    return w1 * w2 * w3;
}
